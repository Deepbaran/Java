...
public class TodoBusinessImplMockitoInjectMockTest {
	...
	// If some framework uses runner [@RunWith] then using the runner of JUnit will not be possible. As we can only have one runner. For example, with Rules we can use Parameterized.class with @RunWith
	...
	//With this Mockito will automatically inject the relevant mocks and return a object. So, todoBusinessImpl is a solid object but the TodoService object used to create it is a mocked object (todoServiceMock).
	
	@Captor
	ArgumentCaptor<String> stringArgumentCaptor;
	//Creates a Argument Captor. Used to capture Arguments passed to a method
