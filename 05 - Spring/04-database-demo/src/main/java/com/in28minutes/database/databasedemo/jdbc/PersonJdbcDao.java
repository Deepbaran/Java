...
public class PersonJdbcDao {
	...
	// Spring Autoconfiguration is automatically configuring the JdbcTemplate to H2 Database as the H2 database is in the CLASSPATH and spring.h2.console.enabled=true
	// Database is autoconfigured when we have either a DataSource or a EmbeddedDatabase, which is H2 in this case.
...
//		return jdbcTemplate.query("select * from person", new BeanPropertyRowMapper<Person>(Person.class)); // This returns a ResultSet which is mapped to the Person Class.
		// Each row that is returend is mapped to the Class Person and a new Person object is created.
		// The class on which we are using the BeanPropertyRowMapper, must have a no argument constructor. 
	...
	public int deleteById(int id) {
		return jdbcTemplate.update("delete from person where id=?", new Object[] {id}); //returns how many rows were updated/deleted
	}
