...
class Checker implements Comparator<Edge> {
	...
	public int compare(Edge e1, Edge e2) {
		...
		/*
		 * e2.weight - e1.weight > 0 -> Swap
		 * e2.weight - e1.weight <= 0 -> Don't Swap
		 */
	}
}
...
public class Kruskals_Algorithm {
	...
	private static void kruskals(Edge[] input, int n, int E) {
	    // Sort the input array - ascending order based on weights
	    Arrays.sort(input, 0, E, new Checker());
	    ...
	    // Print MST
	    for(i = 0; i < n - 1; i++) { // MST has only n - 1 edges
